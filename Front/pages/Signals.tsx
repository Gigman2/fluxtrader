...
import { Search, Filter, Calculator, X, TrendingUp, DollarSign, AlertCircle, ChevronRight, Target, ShieldAlert } from 'lucide-react';
...
const Signals: React.FC = () => {
  ...
  const filteredSignals = useMemo(() => {
    return MOCK_SIGNALS.filter(signal => {
      const matchesSearch = signal.symbol.toLowerCase().includes(searchTerm.toLowerCase()) || 
                            MOCK_CHANNELS.find(c => c.id === signal.channelId)?.name.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const metrics = useMemo(() => {
    ...
    if (symbol.includes('JPY')) {
        ...
        multiplier = 10;
        type = 'Pips';
    } else if (['BTC', 'ETH', 'US30', 'NAS100', 'SPX'].some(s => symbol.includes(s))) {
        multiplier = 1;
        type = 'Points';
    } else {
        // Standard Forex
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Live Signals</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Review and trade active signals.</p>
        </div>
            ...
            >
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-full flex flex-col items-center justify-center text-white font-bold text-xs shadow-sm ${
                            signal.type === SignalType.BUY || signal.type === SignalType.LONG 
                            ? 'bg-emerald-500' 
                            : 'bg-rose-500'
                        }`}>
                            <span className="text-[10px] opacity-90">{signal.type}</span>
                            <span className="text-sm">{signal.symbol.replace('USD', '')}</span>
                        </div>
                        <div>
                            <h3 className="font-bold text-slate-900 dark:text-white text-lg">{signal.symbol}</h3>
                            <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                                <span className="font-medium">{getChannelName(signal.channelId)}</span>
                                <span>•</span>
                    ...
                    <div className="flex items-center gap-6">
                         <div className="hidden sm:block text-right">
                            ...
                            <p className="font-mono font-medium text-slate-900 dark:text-white">{signal.entry}</p>
                         </div>
                         <div className="hidden sm:block text-right">
                            <p className="text-xs text-slate-500">Status</p>
                            <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                                signal.status === SignalStatus.ACTIVE ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400' :
                                signal.status === SignalStatus.PENDING ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400' :
            ...
            <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in duration-200">
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                             <span className={`px-2 py-0.5 rounded text-xs font-bold uppercase tracking-wide ${
                                selectedSignal.type.includes('BUY') || selectedSignal.type.includes('LONG')
                                ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400'
                                : 'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-400'
                             }`}>
                                {selectedSignal.type}
                             </span>
                             <h2 className="text-2xl font-bold text-slate-900 dark:text-white">{selectedSignal.symbol}</h2>
                        </div>
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                            Signal by <span className="font-medium text-slate-900 dark:text-white">{getChannelName(selectedSignal.channelId)}</span>
                        </p>
                    </div>
                    <button 
                    ...
                    <div className="p-6 space-y-8">
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 text-center">
                                <div className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-1">Entry</div>
                                <div className="text-xl font-mono font-bold text-slate-900 dark:text-white">{selectedSignal.entry}</div>
                            </div>
                            <div className="p-4 bg-rose-50 dark:bg-rose-900/10 rounded-xl border border-rose-100 dark:border-rose-900/30 text-center relative overflow-hidden">
                                ...
                                <div className="text-xl font-mono font-bold text-rose-700 dark:text-rose-400">{selectedSignal.sl}</div>
                            </div>
                            <div className="p-4 bg-emerald-50 dark:bg-emerald-900/10 rounded-xl border border-emerald-100 dark:border-emerald-900/30 text-center">
                                ...
                                <div className="text-xl font-mono font-bold text-emerald-700 dark:text-emerald-400">{selectedSignal.tp[0]}</div>
                            </div>
                        </div>
                        ...
                        <div className="bg-slate-50 dark:bg-slate-800/30 rounded-xl p-6 border border-slate-200 dark:border-slate-800">
                            <div className="flex items-center gap-2 mb-6 text-slate-900 dark:text-white font-bold text-lg">
                                <Calculator className="text-slate-900 dark:text-white" size={20} />
                                Position Calculator
                            </div>

                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                                <div>
                                    <label className="block text-xs font-medium text-slate-500 mb-2 uppercase">Account Balance</label>
                                    <div className="relative">
                                ...
                                <div>
                                    <label className="block text-xs font-medium text-slate-500 mb-2 uppercase">Risk Percentage</label>
                                    <div className="relative">
                            ...
                            {metrics && (
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                                    <div>
                                        ...
                                        <p className="font-bold text-slate-900 dark:text-white text-lg">${metrics.riskAmount}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-slate-500 mb-1">Stop Loss ({metrics.type})</p>
                                        <p className="font-bold text-rose-600 dark:text-rose-400 text-lg">{metrics.slPips}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-slate-500 mb-1">Reward/Risk</p>
                                        <p className="font-bold text-emerald-600 dark:text-emerald-400 text-lg">1:{metrics.rrRatio}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-slate-500 mb-1">Est. Position</p>
                                        <div className="flex flex-col">
                                            <span className="font-bold text-slate-900 dark:text-white text-lg">{metrics.units} Units</span>
                                            <span className="text-[10px] text-slate-400">~{metrics.lots} Lots</span>
                                        </div>
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="bg-slate-900 text-slate-300 p-4 rounded-lg font-mono text-xs overflow-x-auto whitespace-pre-wrap">
                            <div className="text-slate-500 mb-2 font-sans text-[10px] uppercase font-bold tracking-wider">Original Message</div>
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 flex gap-3">
                    <button 
                        ...
                        onClick={() => { alert('Trade Execution Not Implemented'); }}
                    >
                        <TrendingUp size={20} />
                        Place Trade
                    </button>
