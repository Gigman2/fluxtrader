...
const generateRegexFromSelection = (fullText: string, selection: string): string => {
  ...
  // Look at context before the selection (up to 10 chars)
  const contextBefore = fullText.substring(Math.max(0, selectionIndex - 10), selectionIndex);
  
  // Check for common delimiters like ": ", "@ ", " "
  const delimiterMatch = contextBefore.match(/([:@\s]+)$/);
  
  if (delimiterMatch) {
    const delimiter = delimiterMatch[1].replace(/\s/g, '\\s*').replace(/[:@]/g, '\\$&');
    // Try to generalize the value
    let valuePattern = '(.+)';
    if (!isNaN(Number(selection))) {
        valuePattern = '([0-9.]+)';
    } else if (/^[A-Z]{3,6}$/.test(selection)) {
        valuePattern = '([A-Z]+)';
    }
...
const TemplateBuilder: React.FC = () => {
  ...
  const [sampleText, setSampleText] = useState("GOLD BUY NOW @ 2045.50\nSL: 2040\nTP1: 2050\nTP2: 2055");
  
  const [fields, setFields] = useState<FieldConfig[]>([
    { id: '1', name: 'Symbol', key: 'symbol', type: 'string', method: 'regex', regex: '([A-Z]{2,6}(?:USD|JPY)?)', required: true, description: 'Asset Pair' },
    { id: '2', name: 'Entry Price', key: 'entry', type: 'number', method: 'regex', regex: '@\\s*([0-9.]+)', required: true, description: 'Entry point' },
  ]);
    ...
    fields.forEach(field => {
      try {
        if (field.method === 'regex') {
          ...
          if (field.type === 'array') {
            const matches = [...sampleText.matchAll(new RegExp(field.regex, 'g'))];
            const values = matches.map(m => m[1] ? parseFloat(m[1]) : m[0]).filter(v => !isNaN(v as number));
            ...
            if (match && match[1]) {
               ...
               result[field.key] = { value: null, error: field.required ? 'Not found' : undefined };
            }
          }
        } 
  ...
  const handleAutoDetect = () => {
    ...
    // Heuristic: Symbol
    if (sampleText.match(/([A-Z]{3,6}(?:USD|JPY)?)/)) {
        newFields.push({
            ...
            type: 'string',
            method: 'regex',
            regex: '([A-Z]{3,6}(?:USD|JPY)?)',
            required: true
        });
    }
    
    // Heuristic: Signal Type
    if (sampleText.match(/(BUY|SELL|LONG|SHORT)/i)) {
        newFields.push({
    ...
    // Heuristic: Entry
    if (sampleText.match(/(@|Entry:|EP)\s*([0-9.]+)/i)) {
        newFields.push({
            ...
            type: 'number',
            method: 'regex',
            regex: '(?:@|Entry:|EP)\\s*([0-9.]+)',
            required: true
        });
    }
    
    // Heuristic: SL
    if (sampleText.match(/(SL|Stop)\s*:?\s*([0-9.]+)/i)) {
        newFields.push({
             ...
             type: 'number',
             method: 'regex',
             regex: '(?:SL|Stop)\\s*:?\\s*([0-9.]+)',
             required: true
        });
    }

     // Heuristic: TP
     if (sampleText.match(/(TP|Target)\d?\s*:?\s*([0-9.]+)/i)) {
        newFields.push({
  ...
  const addFieldFromSelection = () => {
     ...
     const selection = sampleText.substring(start, end);
     const regex = generateRegexFromSelection(sampleText, selection);
     
     const newField: FieldConfig = {
         id: Date.now().toString(),
         name: 'New Field',
         key: selection.toLowerCase().replace(/[^a-z]/g, ''),
         type: !isNaN(Number(selection)) ? 'number' : 'string',
         method: 'regex',
         regex: regex,
         required: true
     };
  ...
  const renderFieldEditor = (field: FieldConfig, index: number) => {
    ...
    return (
        <div key={field.id} className={`rounded-lg border transition-all duration-200 ${isExpanded ? 'bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-600 shadow-md' : 'bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:border-slate-300'}`}>
            {/* Header */}
            <div 
            ...
            >
                <div className="flex items-center gap-3">
                    <div className={`p-1.5 rounded ${extraction?.value ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-slate-200 text-slate-500 dark:bg-slate-800 dark:text-slate-400'}`}>
                         {field.type === 'number' ? <Hash size={14} /> : field.type === 'array' ? <List size={14} /> : <Type size={14} />}
                    </div>
                    <div>
                ...
                <div className="flex items-center gap-3">
                    {extraction?.value ? (
                         <span className="text-xs font-mono bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded text-emerald-600 dark:text-emerald-400 max-w-[100px] truncate">
                            {Array.isArray(extraction.value) ? `[${extraction.value.join(', ')}]` : extraction.value}
                         </span>
                    ) : (
            ...
            {isExpanded && (
                <div className="p-4 border-t border-slate-100 dark:border-slate-800/50 space-y-4 bg-slate-50/50 dark:bg-slate-950/30">
                     <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-semibold text-slate-500 mb-1">Field Name</label>
                            <input 
                                value={field.name}
                                onChange={(e) => {
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-slate-500 mb-1">JSON Key</label>
                            <input 
                                value={field.key}
                                onChange={(e) => {
                                    const newFields = [...fields];
                     ...
                     <div className="grid grid-cols-2 gap-4">
                        <div>
                             <label className="block text-xs font-semibold text-slate-500 mb-1">Data Type</label>
                             <select 
                                value={field.type}
                                onChange={(e) => {
                                    const newFields = [...fields];
                     ...
                     <div>
                        <label className="block text-xs font-semibold text-slate-500 mb-1 flex justify-between">
                            ...
                            <a href="https://regex101.com/" target="_blank" rel="noreferrer" className="text-xs text-slate-400 hover:text-slate-600 hover:underline flex items-center gap-1">
                                Test Regex <ExternalLink size={10} />
                            </a>
                        </label>
                        <div className="relative">
                             <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400 font-mono text-xs">/</span>
                             <input 
                                value={field.regex}
                                onChange={(e) => {
                                    const newFields = [...fields];
                            ...
                            />
                             <span className="absolute right-3 top-1/2 transform -translate-y-1/2 text-slate-400 font-mono text-xs">/{field.type === 'array' ? 'g' : 'i'}</span>
                        </div>
    ...
    <div className="h-[calc(100vh-6rem)] flex flex-col gap-6 overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm shrink-0">
         <div className="flex items-center gap-4 w-full sm:w-auto">
            ...
            <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                     ...
                     >
                        {MOCK_CHANNELS.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                     </select>
                </div>
            </div>
         </div>
         <div className="flex gap-2 w-full sm:w-auto">
            ...
            >
                <Wand2 size={16} /> <span className="hidden sm:inline">Smart Detect</span>
            </button>
            <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-slate-900 hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-200 text-white font-bold rounded-lg transition-colors text-sm shadow-sm">
          ...
          <div className="flex-1 flex flex-col gap-4 min-h-0">
                    ...
                    <button 
                        onClick={() => {
                            if (textareaRef.current) {
                                const start = textareaRef.current.selectionStart;
                                const end = textareaRef.current.selectionEnd;
                                if (start !== end) addFieldFromSelection();
                            }
                        }}
                        className="text-xs bg-emerald-50 text-emerald-600 border border-emerald-200 hover:bg-emerald-100 px-2 py-1 rounded flex items-center gap-1 font-medium transition-colors"
                    >
                        <MousePointerClick size={12} /> Create Field from Selection
                    </button>
                </div>
                <div className="p-2">
                    <textarea 
                    ...
                    <p className="text-[10px] text-slate-400 mt-2 px-1">
                        Tip: Highlight any text above and click "Create Field" to auto-generate a rule.
                    </p>
                </div>
             </div>
             ...
             <div className="flex-1 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col min-h-0">
                <div className="p-3 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center shrink-0">
                    <h3 className="text-sm font-bold text-slate-900 dark:text-white">Fields Configuration</h3>
                    <button 
                        onClick={() => setFields([...fields, { id: Date.now().toString(), name: 'New Field', key: 'new_field', regex: '', type: 'string', method: 'regex', required: true }])}
                ...
                <div className="p-4 overflow-y-auto space-y-3 flex-1">
                    {fields.length === 0 ? (
                        <div className="text-center py-8 text-slate-400">
                            ...
                            <button onClick={handleAutoDetect} className="text-sm font-bold text-slate-900 dark:text-white hover:underline mt-2">Auto-Detect Fields</button>
                        </div>
                    ) : (
              ...
              <div className="bg-slate-900 rounded-xl shadow-lg border border-slate-800 flex flex-col h-full overflow-hidden">
                <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-950 rounded-t-xl shrink-0">
                    <h3 className="font-bold text-slate-100 flex items-center gap-2 text-sm">
                        <Play size={16} className="text-emerald-500" /> Extraction Preview
                    </h3>
                    <div className="flex items-center gap-2">
                         <span className="text-[10px] font-mono text-slate-500 uppercase">JSON</span>
                         <Copy size={14} className="text-slate-500 hover:text-slate-300 cursor-pointer" />
                    </div>
                </div>
                ...
                <div className="flex-1 p-4 overflow-y-auto font-mono text-sm space-y-1">
                    ...
                    {Object.entries(preview).map(([key, val], idx, arr) => {
                        ...
                        return (
                        <div key={key} className="pl-4 group hover:bg-slate-800/50 rounded cursor-pointer">
                            <span className="text-purple-400">"{key}"</span>
                            <span className="text-slate-400">: </span>
                            {result.error ? (
                                <span className="text-rose-500 italic">null <span className="text-[10px] opacity-70">/* {result.error} */</span></span>
                            ) : (
                                <span className={typeof result.value === 'number' ? 'text-orange-400' : 'text-emerald-400'}>
                    ...
                    {fields.some(f => f.required && !preview[f.key]?.value) ? (
                        <div className="flex items-start gap-2 text-rose-400 bg-rose-900/20 p-2 rounded border border-rose-900/50">
                            ...
                            <div>
                                ...
                                <p className="opacity-80">Required fields missing: {fields.filter(f => f.required && !preview[f.key]?.value).map(f => f.name).join(', ')}</p>
                            </div>
                        </div>
                    ) : (
